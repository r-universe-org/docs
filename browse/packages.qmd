...
When posting about your package on social media, you can get a pretty social media card featuring important information on the package such as its maintainer, its [keywords](#keywords), its URL, its version, its number of stars, etc.

![](https://ropensci.r-universe.dev/targets/card.png){fig-alt=""}

Here's how to display the card as preview on a social media post:

- Either simply use the package's URL in your social media post, for instance `https://ropensci.r-universe.dev/targets/`. Indeed, the head of the page source has the metadata field `<meta property="og:image" content="https://ropensci.r-universe.dev/targets/card.png">`: therefore, the image is automatically used by all major social media platforms as the “preview” when you post a link to your package homepage.

- Or download and use the card at either `/{package}/card.png` or `/{package}/card.svg`, for instance `https://ropensci.r-universe.dev/targets/card.png` (PNG format) or `https://ropensci.r-universe.dev/targets/card.svg` (SVG format).

...
For an existing universe, you can create a `packages.json` starter file by calling the [`universe::use_custom_manifest()`](https://docs.ropensci.org/universe/reference/use_custom_manifest.html) function in the [universe R package](https://docs.ropensci.org/universe).

Or you can generate `packages.json` in R from a data frame using [jsonlite](https://jeroen.r-universe.dev/jsonlite):
...
### Can a user have multiple universes or multiple versions of a package?

No, not like that. A universe is a CRAN-like repository, which can only contain one version of each package. And each GitHub user has a single universe. 

If you really want to publish e.g. a development and a production R-universes, you need to manage them from two GitHub organizations. However the more common way is to use publish release versions of your package to a common production universe such as [R-multiverse](https://r-multiverse.org/), or other networks such as CRAN or Bioconductor.
...
To add this badge, you can also use the [`usethis::use_r_universe_badge()`](https://usethis.r-lib.org/reference/badges.html) function:

```r
usethis::use_r_universe_badge("r-lib/cpp11")
```
